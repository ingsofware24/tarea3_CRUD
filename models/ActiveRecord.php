...
class ActiveRecord
{
    ...
    public static function find($id = [])
    {
        ...
        if (is_array(static::$idTabla)) {
            foreach (static::$idTabla as $value) {
                $query .= ($value === reset(static::$idTabla)) ? " WHERE $value = " . self::$db->quote($id[$value]) : " AND $value = " . self::$db->quote($id[$value]);
            }
        } else {
    ...
    public function actualizar()
    {
        ...
        if (is_array(static::$idTabla)) {
            foreach (static::$idTabla as $value) {
                $query .= ($value === reset(static::$idTabla)) ? " WHERE $value = " . self::$db->quote($this->$value) : " AND $value = " . self::$db->quote($this->$value);
            }
        } else {
    ...
    // Eliminar un registro - Toma el ID de ActiveRecord
